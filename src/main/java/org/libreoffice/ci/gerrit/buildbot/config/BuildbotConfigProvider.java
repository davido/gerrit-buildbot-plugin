...
public class BuildbotConfigProvider implements Provider<BuildbotConfig> {
    ...
    private static final String KEY_BUILDBOT_USER_GROUP_NAME = "buildbotUserGroupName";
    ...
    public BuildbotConfig get() {
        ...
        Preconditions.checkNotNull(buildbotAdminGroupName, String.format(
                "%s must not be null", KEY_BUILDBOT_ADMIN_GROUP_NAME));
        config.setBuildbotAdminGroupId(getGroup(buildbotAdminGroupName));

        String buildbotUserGroupName = cfg.getString(SECTION_USER, null,
                KEY_BUILDBOT_USER_GROUP_NAME);
        Preconditions.checkNotNull(buildbotUserGroupName, String.format(
                "%s must not be null", KEY_BUILDBOT_USER_GROUP_NAME));
        ...
        Preconditions.checkNotNull(publisherModeStr,
                "log.mode must not be null");
        LogPublisherMode publisherMode = LogPublisherMode
                .valueOf(publisherModeStr.toUpperCase());
        ...
        if (publisherMode == LogPublisherMode.INTERN) {
            ...
            config.setExternalLogViewerHost(cfg.getString(SECTION_LOG, null,
                    KEY_HOST));
            config.setExternalLogViewerUrl(cfg.getString(SECTION_LOG, null,
                    KEY_URL));
            config.setExternalLogViewerJob(cfg.getString(SECTION_LOG, null,
                    KEY_JOB));
    ...
    private List<BuildbotProject> allProjects(BuildbotConfig config,
            FileBasedConfig cfg) {
    ...
    private BuildbotProject parseProject(BuildbotConfig config,
            FileBasedConfig cfg, String name) {
        ...
        TriggerStrategy triggerStrategie = TriggerStrategy.valueOf(strStrategie
                .toUpperCase());
        ...
        String buildbotAdminGroupName = cfg.getString(SECTION_PROJECT, name,
                KEY_BUILDBOT_ADMIN_GROUP_NAME);
        ...
        String buildbotUserGroupName = cfg.getString(SECTION_PROJECT, name,
                KEY_BUILDBOT_USER_GROUP_NAME);
        p.setBuildbotUserGroupId(buildbotUserGroupName == null ? config
                .getBuildbotUserGroupId() : getGroup(buildbotUserGroupName));
        return p;
    }

    private AccountGroup.UUID getGroup(String groupName) {
        Preconditions.checkNotNull(groupName, "Group name must not be null");
        try {
